...
export default function PopularDestinations() {
  ...
  // ⭐ Default active for row-1 (id:1) and row-2 (id:5)
            ...
            {destinations.slice(0, 3).map((destination) => {
              ...
              return (
                <Link
                  ...
                  onMouseEnter={() => !destination.all && setActiveCardRow1(destination.id)}
                  onMouseLeave={() => setActiveCardRow1(1)} // ⭐ Reset to card 1
            ...
            {destinations.slice(3).map((destination) => {
              ...
              return (
                <Link
                  key={destination.id}
                  href={destination.all ? '/locations' : `/tours?location=${destination.name}`}
                  onMouseEnter={() => !destination.all && setActiveCardRow2(destination.id)}
                  onMouseLeave={() => setActiveCardRow2(6)} // ⭐ Reset to card 5 (last normal card)
                  className={`relative rounded-[16px] overflow-hidden transition-all duration-700 isolate
                    ${isActive ? 'flex-[2]' : 'flex-[1]'}
                    h-[277px]`}
                >
