...
// Interface for Car Bookings (remains the same)
...
// FIXED: Updated TourBooking interface to match your database schema
interface TourBooking {
  ...
  total: number;
  paymentMethod: string;
  paymentStatus: string; // Assuming this comes from the API, as your UI uses it
  firstName: string;
  lastName: string;
  email: string;
  phone: string;
...
// This union type now works with both correct interfaces
...
const BookingHistory = () => {
  ...
  // This type guard function remains the same
  ...
  const handleCancel = (id: string, type: string) => {
    // You would replace this with your actual cancellation logic
    console.log(`Cancel ${type} ID: ${id}`);
    toast(`Cancellation for ${type} ${id} initiated.`); // FIXED: Changed from toast.info
  };

  const handleDownload = (id: string, type: string) => {
    console.log(`Download ticket for ${type} ID: ${id}`);
    toast.success(`Downloading ticket for ${type} ${id}.`);
  };

  const handleReschedule = (id: string, type: string) => {
    console.log(`Reschedule ${type} ID: ${id}`);
    toast(`Rescheduling for ${type} ${id}.`); // FIXED: Changed from toast.info
  };
    ...
    <div className="bg-white rounded-2xl shadow-lg p-8">
      ...
      <div className="space-y-6">
        {allBookings.map((booking) => {
          ...
          // FIXED: Normalize traveler data
          // If it's a car booking, use the nested traveler object
          // If it's a tour booking, create a traveler object from the flat properties
            ...
            >
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      {isCarBooking(booking)
                        ...
                        : booking.pickupPlace} {/* FIXED: Use pickupPlace */}
                    </p>
                  </div>
                </div>
              ...
              >
                <div className="bg-gray-50 rounded-xl p-6">
                  ...
                  {/* Passenger Details - This section now works for both types */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-gray-700">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <span>
                        ...
                        {/* FIXED: Removed hardcoded number */}
                      </span>
                    </div>
                    <div className="flex items-center space-x-2">
                      ...
                      <span>
                        ...
                        {booking.supplier || 'N/A'} {/* FIXED: Removed hardcoded name */}
                      </span>
                    </div>
                  </div>
